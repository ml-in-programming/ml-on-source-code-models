...
CONFIG = {
    ...
    'experiment_notes': "use features without 5 with least mutual info and "
                        "train on part of train set and check train and test accuracy",
    ...
    'features_to_remove_according_to_mutual_info': 5
}
...
def remove_features_with_least_mutual_info(features, number_to_remove):
    mutual_info = mutual_info_classif(features, INPUT_LABELS,
                                      discrete_features=CONFIG['discrete_features'],
                                      random_state=CONFIG['random_state'])
    sorted_features = np.argsort(mutual_info)
    return features[:, sorted_features[CONFIG['features_to_remove_according_to_mutual_info']:]]
...
def get_accuracies(file_to_print):
    ...
    sifted_features = remove_features_with_least_mutual_info(INPUT_FEATURES,
                                                             CONFIG['features_to_remove_according_to_mutual_info'])
    ...
    full_train_index, test_index = next(CONFIG['cv'].split(sifted_features, INPUT_LABELS))
    ...
    for train_percentage in CONFIG['train_splits']:
        ...
        train_features, train_labels = sifted_features[train_index], INPUT_LABELS[train_index]
        test_features, test_labels = sifted_features[test_index], INPUT_LABELS[test_index]
        ...
        model = Model(input_dim=sifted_features.shape[1])
...
def conduct_partial_train_test_accuracy_with_mutual_info_experiment():
...
if __name__ == '__main__':
    conduct_partial_train_test_accuracy_with_mutual_info_experiment()
